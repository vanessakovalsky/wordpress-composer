...
function kovalibre_content_nav( $nav_id ) {
    ...
    if ( is_single() ) {
        $previous = ( is_attachment() ) ? get_post( $post->post_parent ) : get_previous_post(false);
...
<?php next_post_link(); ?>
        <?php //previous_post_link( '<div class="nav-previous">%link</div>', '<span class="meta-nav">' . _x( '&larr;', 'Previous post link', 'kovalibre' ) . '</span> %title', false ); ?>
        <?php //next_post_link( '<div class="nav-next">%link</div>', '%title <span class="meta-nav">' . _x( '&rarr;', 'Next post link', 'kovalibre' ) . '</span>', false ); ?>
